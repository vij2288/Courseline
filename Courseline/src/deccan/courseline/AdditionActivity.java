	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		search.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				String user_fname = "https://dl.dropboxusercontent.com/u/102619389/"
						+ cID + "_users.xml";
				Log.d("ADDITION", "url is : " + user_fname);
				RemoteUtil rm = new RemoteUtil();
				AsyncTask<String, Void, String> res = rm.execute(user_fname,
						"Users");
				String u_fname = Environment.getExternalStorageDirectory()
						.getPath() + "/Download/" + cID + "_users.xml";

				try {
					if (res.get().equals("Success")) {
						if (LocalUtil.checkUsersFile(userID, u_fname)) {

							Course course = mdb.selectCourse(searchText
									.getText().toString());
							if (course != null) {
								...
								rm = new RemoteUtil();
								AsyncTask<String, Void, String> result = rm
										.execute(url, "Course");
								try {
									...
									if (result.get().equals("Success")) {
										...
										b1.setOnClickListener(new OnClickListener() {
											...
											public void onClick(View arg0) {
												...
												System.out
														.println("Static file path: "
																+ filename);
												LocalUtil.ImportCourseData(
														student, filename);
												if (student != null) {
						...
						// remove users file
						File file = new File(u_fname);
						file.delete();
					} else {
						Toast toast = Toast.makeText(getBaseContext(),
								"Course not found.", Toast.LENGTH_LONG);
						toast.setGravity(Gravity.CENTER, 0, 0);
						toast.show();
					}
